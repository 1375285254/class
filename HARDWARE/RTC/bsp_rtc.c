...
extern uint8_t k;
void Time_Show1(struct rtc_time *tm)
{	 
	  /* Infinite loop */
//	  while (1)
//	  {
	    ...
	    {
				/* Display current time */
	      Time_Display1( RTC_GetCounter(),tm); 		  
	      TimeDisplay = 0;
	    }
//	  }
}
...
//  int  aaaa= 0;
	  ...
	  printf(" UNIX时间戳 = %d 当前时间为: %d年(%s年) %d月 %d日 (星期%s)  %0.2d:%0.2d:%0.2d\r",TimeVar,
	                    ...
	                    tm->tm_min, tm->tm_sec);
//    t_tm = (time_t)RTC_GetCounter();;
//    t_tm->tm_year= tm->tm_year;
//    t_tm->tm_mon= tm->tm_mon;
//    t_tm->tm_mday= tm->tm_mday;
//    t_tm->tm_hour= tm->tm_hour;
//    t_tm->tm_min= tm->tm_min;
//    t_tm->tm_sec= tm->tm_sec;
   times_S =  mktime(t_tm)/100;
}



void Time_Display1(uint32_t TimeVar,struct rtc_time *tm)
{
	   static uint32_t FirstDisplay = 1;
	   uint32_t BJ_TimeVar;
	   uint8_t str[15]; // 字符串暂存  	
